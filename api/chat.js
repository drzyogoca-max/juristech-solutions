...
async function handleNodeRequest(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({ status: 'ok', service: 'JurisTech AI Legal Advisor Engine' });
  }
...
function generateHighPrecisionStatutorySynthesis(userMessage, lang) {
  ...
  // 0. Power of Attorney / Wakala (وكالة عامة / توكيل / تفويض)
  if (/(power of attorney|poa|agency|wakala|mandate|وكالة|توكيل|تفويض|وكالة عامة|وكالة خاصة)/i.test(p)) {
