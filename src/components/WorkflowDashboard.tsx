...
export const WorkflowDashboard: React.FC = () => {
  ...
  const steps = [
    {
      stepNum: l('الخطوة 1', 'Step 1'),
      title: l('مراجعة العقد', 'Contract Review'),
      description: l(
    ...
    {
      stepNum: l('الخطوة 2', 'Step 2'),
      title: l('كشف المخاطر', 'Risk Detection'),
      description: l(
    ...
    {
      stepNum: l('الخطوة 3', 'Step 3'),
      title: l('أسئلة استفسارية', 'Legal AI Inquiry'),
      description: l(
    ...
    {
      stepNum: l('الخطوة 4', 'Step 4'),
      title: l('تصدير الملخص', 'Export Summary'),
      description: l(
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'} lang={lang} className="w-full space-y-6 my-6 font-sans">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-gradient-to-r from-slate-900 via-slate-900 to-slate-950 border border-slate-800 rounded-3xl shadow-xl">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 text-xs font-bold uppercase tracking-wider mb-2">
            ...
            <span>{l('مسار العمل المهني المتسلسل', 'Professional Sequential Workflow')}</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-black text-white">
            {l('نظام سير العمل المستند للعقود والمخاطر', 'Workflow-Based Legal Suite')}
          </h2>
        ...
        <div className="flex items-center gap-2">
          <span className="px-3 py-1.5 rounded-xl text-xs font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5">
            ...
            <span>{l('نظام التوافق النافذ نشط', 'Statutory Flow Active')}</span>
          </span>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs font-bold text-cyan-500 group-hover:text-cyan-400">
                <span>{l('البدء بالمرحلة', 'Start Step')}</span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 pt-2">
        <Link
          to="/contracts"
          aria-label={l('رفع عقد جديد', 'Upload New Contract')}
        ...
        >
          ...
          <span>{l('رفع عقد جديد', 'Upload New Contract')}</span>
        </Link>

        <Link
          to="/chat"
          aria-label={l('طرح سؤال قانوني', 'Ask Legal Question')}
        ...
        >
          ...
          <span>{l('طرح سؤال قانوني', 'Ask Legal Question')}</span>
        </Link>

        <Link
          to="/reports"
          aria-label={l('توليد تقرير شامل', 'Generate Report')}
          className="flex items-center justify-center gap-2 p-3.5 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-slate-200 font-bold text-xs sm:text-sm border border-slate-700 transition-all shadow-md active:scale-95"
        >
          <Download className="w-4 h-4 text-emerald-400" />
          <span>{l('توليد تقرير شامل', 'Generate Report')}</span>
        </Link>

        <Link
          to="/risk"
          aria-label={l('مراجعة ملخص المخاطر', 'Review Risk Summary')}
          className="flex items-center justify-center gap-2 p-3.5 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-slate-200 font-bold text-xs sm:text-sm border border-slate-700 transition-all shadow-md active:scale-95"
        >
          <ShieldAlert className="w-4 h-4 text-amber-400" />
          <span>{l('مراجعة ملخص المخاطر', 'Review Risk Summary')}</span>
        </Link>
      </div>
    </div>
  );
